...
BACKGROUND_SOURCE_COLLECTION_NAME = (
    "Background_Source_Models"  # Collection to hold the original imported model
)
    ...
    try:
        ...
        if bpy.context.scene.collection.name in bpy.data.collections:
            ...
            bpy.ops.object.select_all(
                action="DESELECT"
            )  # Deselect all to ensure no active objects affect import
        ...
        print_debug_info(
            f"Found {len(newly_imported_objects)} newly imported objects after import operation."
        )

        if not newly_imported_objects:
            print(
                "Error: No objects were imported or identified. Check file path and type."
            )
            return None
        ...
        new_collections_from_import = [
            ...
            if name not in collections_before_import
            and bpy.data.collections[name].users > 0
        ]
        ...
        for temp_coll in new_collections_from_import:
            ...
            if (
                temp_coll.name != source_collection.name
                and not temp_coll.objects
                and temp_coll.name != "Collection"
            ):
                print_debug_info(
                    f"Removing empty auto-generated collection: {temp_coll.name}"
                )
        ...
        print_debug_info(
            f"Source collection '{BACKGROUND_SOURCE_COLLECTION_NAME}' hidden."
        )

        # Return the top-level objects from the source collection for duplication
        top_level_source_objects = [
            obj for obj in source_collection.objects if obj.parent is None
        ]

        # Print object names for debugging AFTER applying transforms
        for obj in top_level_source_objects:
            print_debug_info(
                f"Source object: {obj.name} (type: {obj.type}) - Loc:{obj.location} Rot:{obj.rotation_euler} Scale:{obj.scale}"
            )
...
class BackgroundGridGeneratorOperator(bpy.types.Operator):
        ...
        if existing_grid_parent and existing_grid_collection:
            ...
            if has_existing_grid_content and not should_rebuild:
                ...
                for obj in list(self._grid_collection.objects):
                    if (
                        obj == self._grid_parent
                    ):  # Don't delete the master parent itself
                        continue
                    ...
                    for coll in obj.users_collection:
                        if (
                            coll != self._grid_collection
                            and coll != bpy.context.scene.collection
                        ):
            ...
            for coll in list(self._grid_parent.users_collection):
                if (
                    coll != self._grid_collection
                ):  # Don't unlink from its own collection or the scene collection
    ...
    def finish_operation(self, context):
        ...
        try:
            # Set viewport clip end to 100,000 for distant backgrounds
